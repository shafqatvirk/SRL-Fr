...
def generate_zimple_program(sent):
	...
	for f in frames:
		...
		uniqueStr = uniqueStr + 'subto unique'+str(idx)+ ': forall <r> in Roles["'+f+'"] do sum <s> in Spans["'+f+'"] : z'+str(idx)+'[r,s] == 1;\n'
		gStr = gStr + 'set G'+str(idx)+'[I] := ' + buildSpanOverlapingSet(spans[f])
		overlapStr = overlapStr + 'subto overlap' +str(idx)+ ': forall <i> in I do \n sum <r> in Roles["'+f+'"] do \n sum <s> in G'+str(idx)+'[i] do z'+str(idx)+'[r,s] <= 1;\n'
	...
	f2fSubRelationConstraints(frames,roles)
	
	output_program_file.write('set Roles[Frames] := '+rStr)
	output_program_file.write('set Spans[Frames] := '+sStr)
	output_program_file.write('set Assignment[<f> in Frames] := {Roles[f]*Spans[f]};\n')
	...
	output_program_file.write('#Overlab Constraints\n')
...
def f2fSubRelationConstraints(frames,roles):
	for i in range(0,len(frames)):
		for j in range(i+1,len(frames)):
			if j < len(frames):
				print (frames[i],frames[j])
